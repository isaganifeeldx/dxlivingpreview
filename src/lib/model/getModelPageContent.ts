...
function mapModelFromCms(
  doc: CmsModel | null | undefined,
  moduleCards: ModelPageCmsContent['moduleCards'],
): ModelPageCmsContent {
  const defaults = modelPageDefaults
  if (!doc) {
    return {
      ...defaults,
      moduleCards,
  ...
  const featuresList: ModelFeatureItem[] =
    doc.features?.list
      ?.map((item, index) => {
      ...
      .filter((item): item is ModelFeatureItem => Boolean(item)) ?? []

  const anchorMenu: ModelAnchorMenuItem[] = []
  for (let index = 0; index < MODEL_ANCHOR_SECTION_IDS.length; index += 1) {
    const sectionId = MODEL_ANCHOR_SECTION_IDS[index]
...
function otherModuleCards(
  cards: ModelPageCmsContent['moduleCards'],
): ModelPageCmsContent['moduleCards'] {
  return cards.filter((card) => card.link !== '/model').slice(0, 3)
}

async function getModelModuleCards() {
...
export const getModelPageContent = cache(async (): Promise<ModelPageCmsContent> => {
  const moduleCards = await getModelModuleCards()
  ...
  try {
    ...
    return mapModelFromCms(doc, moduleCards)
  } catch (error) {
    console.error('[model] Failed to load Model global from Payload — using defaults.', error)
    return {
      ...FALLBACK_MODEL_CONTENT,
      seo: modelPageDefaults.seo,
      moduleCards,
    }
  }
})
